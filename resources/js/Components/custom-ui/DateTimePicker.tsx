...
function genMonths(locale: Pick<Locale, "options" | "localize" | "formatLong">) {
  return Array.from({ length: 12 }, function (_, i) {
    const month = format(new Date(2025, i), "MMMM", { locale }).toLowerCase();
...
function genYears({ startYear, endYear }: { startYear: number; endYear: number }) {
  const today = new Date();

  const yearStart = today.getFullYear() - startYear;
  const yearEnd = today.getFullYear() + endYear;
...
function Calendar({
  ...
  endYear,
  ...props
}: CalendarProps & { startYear?: number; endYear?: number | false }) {
  ...
  const YEARS = React.useMemo(() => genYears({ startYear, endYear: endYear || 0 }), []);
